...
/// マイクは音量のように「ハードを最大にしてデジタルで絞る」形にはできない。
/// 常時鳴っている周囲の雑音がしきい値
/// （`core::turn_detector::VOICE_THRESHOLD`）を超え続けると、声のあとの
/// 沈黙が一度も来なくなり、`core::turn_detector::MAX_DURATION_MS` の
/// 上限までいつも録音し続けてしまう（実機で確認）。そのため基準値は
/// コーデックの最大（42dB）ではなく、実機で沈黙をしきい値6と安定して
/// 区別できると確かめた36dB（[仕様](../../docs/spec.md#未確定の項目)参照）
/// に置き、そこへ0〜100%（0〜+12dB）のデジタルゲインを足す形にする。
const FIXED_MIC_GAIN_DB: f32 = 36.0;
