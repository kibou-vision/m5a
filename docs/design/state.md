...
5分（`IDLE_SHUTDOWN_MS`）経ったことを検知すると `Idle` を起こす。
...
stateDiagram-v2
  ...
  Assistant --> Settings : SwipedToSettings（下スワイプ）
  Settings --> Assistant : SwipedToAssistant（上スワイプ）
...
設定画面の開閉は専用のボタンを持たず、上下スワイプだけで行う
（[設定画面の設計](settings.md#スワイプ判定)参照）。
