...
音声を結線する際に確認が必要な点。BSP の
`bsp_audio_codec_speaker_init` / `bsp_audio_codec_microphone_init` を使えば
コーデックのレジスタ列と初期化順序は BSP が引き受ける。

* マイクとスピーカーは BCLK/WS を共有するため、標本化周波数を揃える必要がある
* AW88298 は 44.1kHz だと PLL がロックせず無音になる
* 録音は専用の仕事で回す。描画と同じ流れで読むと取りこぼす
* `EspWebSocketClient` は受信バッファを超えるフレームを組み立て直せない
  （esp-idf-svc の既知の問題）。μ-law なら断片が小さく、この制約に当たりにくい
