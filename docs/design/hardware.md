...
## 電源を落とすのは deep sleep で代用する（AXP2101 直接シャットダウンは断念）

自動シャットダウン（[状態遷移](state.md)の `Idle` → `ShuttingDown`）は
最初 ESP-IDF 標準の `esp_deep_sleep_start()` だけで済ませようとしたが、
実機で複数の問題が見つかり、AXP2101 のレジスタを直接叩いて本体ごと
電源を切る方式を試したものの、最終的に**Baseモジュール（バッテリー
台座）を使う運用では実現不可能**と判明し、deep sleep による代用に
戻した（`hal::board::power_off`）。
...
`bsp_display_backlight_off()` でバックライトを消し、
`bsp_display_enter_sleep()` でパネルを休止させるようにした
（どちらも BSP の標準 API）。

**問題2: それでも実機では「電源を切ったはずが再起動する」症状が
再現し続けた。** 経路は2つあった。ひとつは `bsp_display_enter_sleep()`
が LVGL を介さず `esp_lcd_panel_disp_on_off()` を直接呼ぶため、LVGL の
描画タスクが同じ SPI バスへ同時にフレームを流していると衝突すること
（`DisplayLock::acquire()` で描画タスクを止めてから呼ぶよう修正した）。
もうひとつ、より根本的な原因として、ESP32-S3 の deep sleep はそもそも
**起きるとブートローダーからやり直す**ため、起床要因を一切設定して
いなくても何かの拍子に起きてしまえば見た目は「電源を切った」のではなく
「再起動した」にしかならない。この意味で deep sleep は本当の意味での
電源断ではない。

**そこで AXP2101 自身にシャットダウンさせる方式を試した。**
AXP2101 の「共通設定」レジスタ（0x10）の bit0 に1を立てると、RTC用の
VRTC を除くすべてのレール（ESP32-S3 本体の3.3V系も含む）が切れる。
実機の M5Stack 製品群で使われている
...
**問題2.5: USBを完全に抜いても再起動が再現し続けた。** AXP2101の
データシート（`6.5.4.2 Power on`）には電源投入要因のひとつとして
「VBUS low → high」がある。M5Stackの各シリーズでは、Baseモジュール
（バッテリー台座）が M-Bus の5Vピンを通じて本体へ給電しており、この
5VはAXP2101から見ると USB の VBUS と区別が付かない。ソフトウェアで
`REG10H[0]` を立てて電源を切ると内部状態がリセットされ、その直後にも
まだ生きているBaseモジュールからの給電を「VBUSが新たに挿さった」と
検知して即座に電源を入れ直してしまう。USBケーブルの有無に関わらず、
**Baseモジュールを本体に挿している限りこの電源断は成立しない**——
これは配線上の制約であり、レジスタの選び方やI2Cの実装では回避できない。

そのため AXP2101 レジスタでの電源断は断念し、
`esp_deep_sleep_start()`（バックライトを消し、パネルを休止させた状態）
による代用に戻した。起床要因はあえて設定しない。設定すると
「切ったつもりが何かの拍子に動き出す」という分かりにくい挙動になるため、
目覚めは実機の電源ボタンによる起動に任せる。

**問題3: AXP2101 で本当に電源を切れるようになったら、直前に保存した
設定が次回起動に反映されないことがある。** 明るさの保存で実機を通して
確認した。`fs::write` が成功を返し、書き戻した直後の読み戻しでも
一致していても、SDカードの書き込みキャッシュにデータが残ったまま
AXP2101 で即座に電源を断つと、そのぶんが失われてしまう。
`esp_deep_sleep_start()` だけだった頃は実際には電源が落ちておらず
（問題2）この危険が表面化していなかった。

対策は二段構え。まず `power_off()` の中で AXP2101 に切らせる前に
`bsp_sdcard_unmount()`（`hal::board::unmount_sd_card()`）を呼び、
ファイルシステムを明示的に同期させてから電源を断つ。ただし
自動シャットダウンは無操作から3分待つため、実際には給電を直接
断たれる（電池を抜く、電源ボタンの長押しなど）ことの方が多く、
その経路は事前に何も呼べない。そこで本丸として `hal::storage::SdStorage`
が書き込むたびに `File::sync_all()` を呼び、`power_off()` を経由するか
どうかに関わらず、書いたその場でディスクへ確実に落とすようにした。
