...
**問題2（本当の原因）: `bsp_display_enter_sleep()` が呼ぶたびに必ず
パニックで再起動していた。** パネルもろとも休止させようとこの BSP 標準
API を追加したところ、実機のパニックログに次が出た。
...
**問題3: 電源断の途中で、直前に保存した設定が次回起動に反映されない
ことがあった。** 明るさの保存で実機を通して確認した。`fs::write` が
成功を返し、書き戻した直後の読み戻しでも一致していても、SDカードの
書き込みキャッシュにデータが残ったまま給電が断たれると、そのぶんが
失われてしまう。

対策は二段構え。まず `power_off()` の中で deep sleep に入る前に
`bsp_sdcard_unmount()`（`hal::board::unmount_sd_card()`）を呼び、
ファイルシステムを明示的に同期させておく。ただし自動シャットダウンは
無操作から3分待つため、実際には給電を直接断たれる（電池を抜く、
電源ボタンの長押しなど）ことの方が多く、その経路は事前に何も呼べない。
そこで本丸として `hal::storage::SdStorage` が書き込むたびに
`File::sync_all()` を呼び、`power_off()` を経由するかどうかに関わらず、
書いたその場でディスクへ確実に落とすようにした。
